CREATE TABLE `agent` (
  ...
  `type` tinyint(2) DEFAULT '0' COMMENT '显示类型 0 园区座席 1 全网座席',
...
DROP INDEX `unique_device_rtsp`,
ADD UNIQUE INDEX `unique_device_rtsp`(`rtsp_url`, `name`) USING BTREE COMMENT 'rtsp_url不重复';
...
CREATE TABLE `channel_device_relation` (
  ...
  `type` tinyint(2) DEFAULT '0' COMMENT '设备的类型 0: 全局摄像头, 1: 车牌识别摄像头',
  `create_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `time_version` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '数据的版本',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;